...
!use mtmod
 ...
 implicit none
  ...
  real(8) :: tvar !! variable used to store intermediate temperature
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!! array that will hold all the sites in the cluster!!!
  ...
  real(8),dimension(0:n_sites-1):: m   !! m
  real(8),dimension(0:n_sites-1):: theta !! theta
  real(8),dimension(0:n_sites-1):: phi  !! phi
...
!   call MPI_COMM_SIZE(MPI_COMM_WORLD,num_procs,ierr)
    ...
    !! subroutined to initialize the mc variables(m,theta,phi) at each site
    call mcvar_init(n_sites,m,theta,phi,pi,m_min,m_max)

    !! subroutine to initialize the full lattice hamiltonian (2L*L,2*L*L)
...
!!-----------------------------------------------------------------------------!!
 subroutine print_f(fname,u_int,tvar,L,cls_sites)
   ...
   fname=trim('configurations_L')//trim(str_1)//trim('_temp')//trim(str_2)//trim('_Uint')&
